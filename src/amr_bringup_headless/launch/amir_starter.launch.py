...
def generate_launch_description():
    ...
    # Tambahan konfigurasi untuk 2 package baru
    line_delay = LaunchConfiguration('line_delay')
    obstacle_delay = LaunchConfiguration('obstacle_delay')
    ...
    magnetic_line_follower_share = get_package_share_directory('magnetic_line_follower')
    ccf_laser_sensor_share = get_package_share_directory('ccf_laser_sensor')
    ...
    magnetic_line_follower_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                magnetic_line_follower_share,
                'launch',
                'magnetic_line_follower_integrated.launch.py'
            )
        ),
        launch_arguments={
            'port': '/dev/line',
    ...
    ccf_laser_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                ccf_laser_sensor_share,
                'launch',
                'ccf_laser.launch.py'
            )
        ),
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('line_delay', default_value='4.0'),
        DeclareLaunchArgument('obstacle_delay', default_value='5.0'),
        ...
        # =========================
        # 10. LINE FOLLOWER & OBSTACLE LASER
        # =========================
        TimerAction(
            period=line_delay,
            actions=[magnetic_line_follower_launch]
        ),

        TimerAction(
            period=obstacle_delay,
            actions=[ccf_laser_launch]
        ),
    ])
