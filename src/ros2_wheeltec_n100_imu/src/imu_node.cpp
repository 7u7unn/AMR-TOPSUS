  ...
  void read_imu()
  {
    ...
    while(rclcpp::ok())
    {
      ...
      if (head_type[0] == TYPE_AHRS)
      {
        ...
        // ✅ FIX 1: Copy header (timestamp + frame_id)
        imu_trueEast_data.header = imu_data.header;

        // ✅ FIX 2: Copy angular velocity values
        imu_trueEast_data.angular_velocity = imu_data.angular_velocity;
        imu_trueEast_data.linear_acceleration = imu_data.linear_acceleration;
