...
def check_headless_support():
    ...
    if not app_js.exists():
        app_js = Path("arya_web_interface/static/js/app.js")
    if not app_js.exists():
    ...
    endpoints = [
        ("/api/topics", "Topics API endpoint"),
        ("/api/maps", "Maps list endpoint"),
        ("/api/nav_annotations", "Navigation annotations API"),
...
def check_function_conflicts():
    ...
    if not js_dir.exists():
        js_dir = Path("arya_web_interface/static/js")
    ...
    expected = {
        "updateMissionStatusUi", "selectedMapName", "onNavPath", "driveMode", "onNavGlobalCostmap",
        "onNavMap", "drawFixedAxesAndRobot", "setLidarMotorUi", "onNavLocalCostmap", "data",
        "amclData", "launchStates", "navGoalStatus", "availableMaps", "updateMissionAckUi",
        "setDriveModeUi", "requestNavDraw", "onNavLidarScan"
    }
